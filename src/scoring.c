...
float score_q7_to_float(score_q7_t amount) { return ((float)amount) / ((float)(FLOAT_TO_SCORE_Q7)); }

score_q7_t float_to_score_q7(float amount) { return (score_q7_t)(amount * FLOAT_TO_SCORE_Q7); }

score_q7_t score_q7(const state *s) {
  return (simple_area_score(s) * FLOAT_TO_SCORE_Q7 + s->button * BUTTON_Q7 + s->ko_threats * KO_THREAT_Q7);
}
float score(const state *s) { return score_q7_to_float(score_q7(s)); }

score_q7_t target_lost_score_q7(const state *s) {
  return (-TARGET_CAPTURED_SCORE_Q7 + s->button * BUTTON_Q7 + s->ko_threats * KO_THREAT_Q7);
}
float target_lost_score(const state *s) { return score_q7_to_float(target_lost_score_q7(s)); }

score_q7_t take_target_score_q7(const state *s) {
  return (TARGET_CAPTURED_SCORE_Q7 + s->button * BUTTON_Q7 + s->ko_threats * KO_THREAT_Q7);
}
float take_target_score(const state *s) { return score_q7_to_float(take_target_score_q7(s)); }
...
table_value apply_tactics_q7(const tactics ts, const move_result r, const state *child, const table_value child_value) {
  ...
  case DELAY:
    return (table_value){-delay_capture_q7(child_value.low), -delay_capture_q7(child_value.high)};
  case FORCING:
    if (r > PASS && !child->button) {
      return (table_value){reward_force_q7(-child_value.low), -child_value.high};
    } else {
      return (table_value){-child_value.low, -child_value.high};
    }
  }
...
value apply_tactics(const tactics ts, const move_result r, const state *child, const value child_value) {
  ...
  case DELAY:
    return (value){-delay_capture(child_value.low), -delay_capture(child_value.high)};
  case FORCING:
    if (r > PASS && !child->button) {
      return (value){reward_force(-child_value.low), -child_value.high};
    } else {
      return (value){-child_value.low, -child_value.high};
    }
  }
