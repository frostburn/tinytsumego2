...
/** @brief Large win value used when a group of target stones is captured. */
...
typedef enum tactics {
  ...
  NONE,
  /** @brief Reward delaying inevitable capture of a target. */
  DELAY,
...
/** @brief Score a state where a target has been lost, returned in Q7 form. */
score_q7_t target_lost_score_q7(const state *s);

/** @brief Score a state where a target has been lost. */
float target_lost_score(const state *s);

/** @brief Score a state where a target has been captured, returned in Q7 form. */
score_q7_t take_target_score_q7(const state *s);

/** @brief Score a state where a target has been captured. */
